...
supervisor (supervisor_t *sup) {
  ...
  connection_t conn;
  char sendline[MAXLINE], recvline[MAXLINE], newline[MAXLINE];
  ...
  /* [start] 2021.11.08 yunji, jiyun: recvline과 sendline을 맞춰주기 위한 코드 */

    int num = atoi(sendline);
    int k;
    newline[0]='\0';

    if(num == 0 || num == 1) strcat(newline, "non-prime ");
    else if(num == 2 || num == 3) strcat(newline, "prime ");
    else {
      for (k = 2; k <= num / 2; k++) {
        if (num % k == 0) {
          strcat(newline, "non-prime ");
          break;
        }
      }
    }
    if(k == num / 2 + 1) strcat(newline, "prime ");
    strcat(newline, "\n");

  /* [end]] 2021.11.08 yunji, jiyun: recvline과 sendline을 맞춰주기 위한 코드 */

    if ((strlen (recvline) != strlen (newline)) ||
        strcmp (recvline, newline)) {
